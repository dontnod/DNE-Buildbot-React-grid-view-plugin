import "./DNEFailureDashboard.scss"
...
function getSelectForm(selectManager: DNEProjectBranchSelectManager, builders: Builder[]) {

  const [selectedBuilders, setSelectedBuilders] = useState([]);

  const builderOptions: Option[] = builders.map(b => ({label: b.name, value: b.id}));

  const selectForms = (
    <Form inline className="mb-sm-2">
      <Form.Label className="mr-sm-2">Project</Form.Label>
      <Form.Control className="mr-sm-2" as="select" multiple={false} value={selectManager.getProjectId().toString()}
                    onChange={event => selectManager.setProjectId(event.target.value)}>
        {selectManager.config.projects.map(p => (<option value={p.identifier}>{p.display_name}</option>))}
      </Form.Control>
      <Form.Label className="mr-sm-2">Branch</Form.Label>
      <Form.Control className="mr-sm-2" as="select" multiple={false} value={selectManager.getBranchId()}
                    onChange={event => selectManager.setBranch(event.target.value)}>
        {selectManager.getProjectOrDefault()?.branches.map(branch => (<option value={branch.identifier}>{branch.display_name}</option>))}
      </Form.Control>

      <Form.Label className="mr-sm-2">Builders</Form.Label>
      <MultiSelect
        className="builders-multi-select"
...
// setSelectedBuilders(builders.filter(b => b.id);
  );

  return {form: selectForms};
}
...
function getBuildersFromTag(accessor: IDataAccessor, tag: string) {
  const buildersQuery = useDataApiQuery(() => Builder.getAll(accessor, {query: {order: 'name'}}));
  return buildersQuery
    .array
    .filter((b: Builder) => b.tags.indexOf(tag) >= 0);
}

export const DNEFailureDashboard = observer(() => {
  ...
  const selectManager = new DNEProjectBranchSelectManager(config, searchParams, setSearchParams);

  const builders = getBuildersFromTag(accessor, selectManager.getTag());

  const {form: viewSelectForm} = getSelectForm(selectManager, builders);

  return viewSelectForm;
  // const {
  //   queriesResolved,
  //   builders,
  //   buildrequestsQuery,
...
export default DNEFailureDashboard;
